...
class RenderEngine {
  ...
  static Future<String> saveDocument(Uint8List bytes, String originalPath, {bool isOverwrite = false}) async {
    ...
    if (await targetFile.exists() && isOverwrite) {
  ...
  static List<String> getFormFields(Uint8List bytes) {
    ...
    try {
      ...
      for (int i = 0; i < document.form.fields.count; i++) {
        fields.add(document.form.fields[i].name ?? 'Field_$i');
      }
      return fields;
    } finally {
      document.dispose();
  ...
  /// REFACTORED: Use Template-based merge if importPageRange is missing in this version
  ...
  static Future<Uint8List> addTextAnnotation({
    ...
    try {
      ...
      for (var bound in bounds) {
        ...
        // Correct way to set type in standard SF API
        // annotation.textMarkupAnnotationType = type; // if setter exists
  ...
  static Future<Uint8List> redactArea(Uint8List bytes, int pageIndex, Rect bounds) async {
    ...
    try {
      ...
      // If PdfRedactionAnnotation is missing, visual mask is the fallback for this build
      final List<int> savedBytes = await document.save();
      return Uint8List.fromList(savedBytes);
    } finally {
      document.dispose();
  ...
  static Future<Uint8List> applySecurity(Uint8List bytes, String userPassword, {String? ownerPassword}) async {
    final sf.PdfDocument document = sf.PdfDocument(inputBytes: bytes);
    try {
      document.security.userPassword = userPassword;
      document.security.ownerPassword = ownerPassword ?? userPassword;
      // document.security.encryptionAlgorithm = sf.PdfEncryptionAlgorithm.aes256Bit;
  ...
  static Future<Uint8List> addInkAnnotation({
    ...
    try {
      ...
      // Fallback if PdfInkAnnotation is not in this specific SDK version
      page.annotations.add(sf.PdfRectangleAnnotation(
        Rect.fromLTWH(paths[0][0].dx, paths[0][0].dy, 10, 10),
        'Ink Placeholder',
      ));
  ...
  /// REFACTORED: Template-based reorder
  ...
  static Future<Uint8List> addWatermark(Uint8List bytes, String text) async {
    ...
    try {
      ...
      for (int i = 0; i < document.pages.count; i++) {
        ...
        final double x = pageSize.width / 2;
        final double y = pageSize.height / 2;
        ...
        page.graphics.drawString(
          ...
          font, 
          brush: brush, 
          bounds: const Rect.fromLTWH(-150, -30, 0, 0),
        );
